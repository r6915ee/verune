...
Runtime metadata can be declared using `meta.ron` files, which are in the [RON]
...
version information is stored using [RON]. If we specify a version that is not
installed, however, we get an error. If you're just looking to tell the program
which version you want to use, you may do so by using the `-u`/`--skip-check`
flag.
...
use the system's command line shell (e.g. [Bash]), but other programs and even
